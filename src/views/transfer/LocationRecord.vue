<template>
  <div class="location-record-container">
    <el-card shadow="hover">
      <template #header>
        <div class="card-header">
          <span class="title">储位调整记录</span>
          <div class="actions">
      ...
      <el-form :inline="true" :model="searchForm" class="search-form">
        ...
        <el-form-item label="物料编码">
          <el-input v-model="searchForm.materialCode" placeholder="请输入" clearable />
        </el-form-item>
      ...
      <el-row :gutter="20" class="stats-row">
        ...
        <el-col :span="6">
          <div class="stat-card">
            ...
            <div class="stat-content">
              <div class="stat-label">调整物料数</div>
        ...
        <el-col :span="6">
          <div class="stat-card">
            ...
            <div class="stat-content">
              <div class="stat-label">调整总数量</div>
      ...
      <el-table :data="tableData" border stripe v-loading="loading">
        ...
        <el-table-column prop="orderNo" label="调整单号" min-width="160">
        ...
        <el-table-column prop="adjustType" label="调整类型" width="120">
          <template #default="{ row }">
            <el-tag :type="row.adjustType === 'SINGLE' ? 'primary' : 'success'">
              {{ row.adjustType === 'SINGLE' ? '单品调整' : '批量调整' }}
            </el-tag>
          </template>
        </el-table-column>
        <el-table-column prop="itemCount" label="物料数" width="90" align="right" />
        <el-table-column prop="totalQty" label="调整数量" width="100" align="right" />
    ...
    <el-dialog 
      ...
      :title="`调整详情 - ${detailDialog.orderNo}`" 
      width="85%"
    >
      <el-descriptions :column="3" border>
        <el-descriptions-item label="调整单号">{{ detailData.orderNo }}</el-descriptions-item>
        <el-descriptions-item label="仓库">{{ detailData.warehouse }}</el-descriptions-item>
        <el-descriptions-item label="调整类型">
          <el-tag :type="detailData.adjustType === 'SINGLE' ? 'primary' : 'success'">
            {{ detailData.adjustType === 'SINGLE' ? '单品调整' : '批量调整' }}
          </el-tag>
        </el-descriptions-item>
        <el-descriptions-item label="调整原因">
          <el-tag :type="getReasonColor(detailData.reason)">{{ getReasonLabel(detailData.reason) }}</el-tag>
        </el-descriptions-item>
        <el-descriptions-item label="操作人">{{ detailData.operator }}</el-descriptions-item>
        <el-descriptions-item label="调整时间">{{ detailData.adjustTime }}</el-descriptions-item>
      ...
      <el-table :data="detailData.items" border max-height="400">
        ...
        <el-table-column prop="fromLocation" label="原储位" width="120">
          <template #default="{ row }">
            <el-tag type="info">{{ row.fromLocation }}</el-tag>
          </template>
        </el-table-column>
        <el-table-column label="调整方向" width="80" align="center">
          <template #default>
            <el-icon color="#409EFF" :size="20"><Right /></el-icon>
          </template>
        </el-table-column>
        <el-table-column prop="toLocation" label="新储位" width="120">
          <template #default="{ row }">
            <el-tag type="success">{{ row.toLocation }}</el-tag>
          </template>
        </el-table-column>
...
import { Document, Box, ShoppingCart, Timer, Right } from '@element-plus/icons-vue'
...
const loadData = async () => {
  loading.value = true
  try {
    const res = await request.get('/transfer/location/records', {
      params: {
...
const handleDetail = async (row) => {
  try {
    const res = await request.get(`/transfer/location/records/${row.id}`)
